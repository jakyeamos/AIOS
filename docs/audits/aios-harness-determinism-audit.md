...
3. **Prompt-time retrieval trace is too thin.** `bin/hook-prompt-submit.py` can inject prompt templates, bugs, archive notes, handoff decisions, wiki snippets, and rules, but `prompts_used` records only `retrieval_fired` and `retrieval_source`.
