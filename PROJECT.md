...
- `agent_eval_contract/` now provides that first in-repo contract boundary without moving runtime/storage out of AIOS. It owns context-profile/status/priority validation, score fields, TypedDict schemas, harness fixture validation, eval template validation, bundled sample record validation, a clean-room contract runner, and external benchmark normalization; `services.eval_run_service`, `services.harness_eval`, `services.external_benchmark_adapter`, and `config/agent-eval/eval-schemas.py` consume it. Physical extraction remains blocked until non-AIOS fixture production and release metadata exist.
