...
- `agent_eval_contract/` now provides that first in-repo contract boundary without moving runtime/storage out of AIOS. It owns context-profile/status/priority validation, score fields, TypedDict schemas, harness fixture validation, eval template validation, bundled sample record validation, a clean-room contract runner, non-AIOS fixture bundle production, checked-in release metadata, and external benchmark normalization; `services.eval_run_service`, `services.harness_eval`, `services.external_benchmark_adapter`, and `config/agent-eval/eval-schemas.py` consume it. Physical extraction remains blocked until a split rehearsal proves tagged AIOS dependency consumption and at least one second consumer.
