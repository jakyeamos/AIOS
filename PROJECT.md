...
- `agent_eval_contract/` now provides that first in-repo contract boundary without moving runtime/storage out of AIOS. It owns context-profile/status/priority validation, score fields, TypedDict schemas, harness fixture validation, and external benchmark normalization; `services.eval_run_service`, `services.harness_eval`, `services.external_benchmark_adapter`, and `config/agent-eval/eval-schemas.py` consume it. Physical extraction remains blocked until sample eval records, template validators, clean-room fixtures, and non-AIOS fixture production exist.
