...
This keeps the current workspace as the baseline source of truth and creates an isolated AIOS shadow worktree for comparison. The wrapper is the narrow permission boundary for shadow setup; it only delegates to `scripts/codex-aios-shadow.py`, whose worktree creation writes Git refs under `.git`. For scored good/excellent candidates, the helper may also launch a headless `codex exec` shadow run inside that isolated worktree using `workspace-write` sandboxing and `--ask-for-approval never`; blocked, small, dirty, unsafe, or unmeasurable tasks stay route-only with a recorded skip reason. The returned AIOS route, packet, shadow metadata, and shadow execution artifacts are evidence only for ordinary tasks; continue the baseline implementation normally unless the user explicitly requested `/aios`. Do not merge, copy, or promote shadow output into the baseline workspace without explicit user review.
...
Treat the returned run, route, packet, shadow metadata, and headless execution artifacts as the task's governing context only for `/aios` prompts. For ordinary prompts, use the returned metadata only as comparison evidence for AIOS usefulness. After implementation, inspect the run with `operator-search`, `daily-flow`, `next-action`, and `aios shadow status --shadow-run-id <id>` using the commands returned by the helper when they exist.
