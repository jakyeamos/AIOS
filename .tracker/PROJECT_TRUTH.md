...
schemaVersion: 1
projectName: AIOS
summary: Active infra project powering Claude Code agent workflows with a committed CTS backend, pyproject-based quality tooling, and broader script cleanup underway; quality gates are explicit but still failing.
healthScore: 61
statusLabel: needs_attention
nextStep: Normalize and commit the command-center UI and remaining standalone utilities, then resume Ruff and BasedPyright remediation with the CTS backend now in-repo.
blockers: []
...
The codebase is large — roughly 60 scripts in `bin/`, one committed test file, and growing docs. `pyproject.toml` is now part of the repo, so Ruff, BasedPyright, and Vulture configuration live in source control instead of only existing locally. The Code Topology Service (CTS) backend is now also committed: `services/cts/` provides graph storage, parsing, search, impact analysis, incremental updates, and MCP/CLI entrypoints, and `hook-session-start.py` can now inject CTS context when an index is current. A fresh quality pass on 2026-04-12 confirmed that Ruff still has 20 remaining issues and BasedPyright has 22 errors plus 60 warnings.
...
5. **Command-center UI and standalone utilities are still local-only** — `aios-ui/`, `bin/aios-query.py`, and `bin/token-audit.py` remain to be normalized into committed units
...
3. Normalize and commit the command-center UI unit and the remaining standalone utility scripts
...
- Large local UI/utility trees still need to be split into coherent commits without mixing generated artifacts
...
| Structure | Pass | `pyproject.toml`, `schema.sql`, `bin/`, `services/`, and `tests/` are organized; CTS is now committed and UI/utility work remains to be normalized |
