...
AIOS now has a strict subsystem ownership map in `.planning/SUBSYSTEM_EXTRACTION_PLAN.md`. The map classifies major surfaces as `core_aios`, `adapter_inside_aios`, `contract_package`, `standalone_tool`, or `incubator_candidate`. The current scope truth is that AIOS is better scoped after extracting `repo-quality-certifier`, `quality-evidence-contract`, `context-compiler-contract`, and the Research Domain Writing standalone tool. CTS, personalized humanizer, and eval/benchmark runtime have now been audited and intentionally remain AIOS-owned for runtime/state reasons. The in-repo `agent_eval_contract` package boundary now covers portable eval schemas, validators, template validation, bundled sample records, clean-room contract checks, non-AIOS fixture bundle production, release metadata, and external normalization; physical extraction remains gated on split rehearsal, tagged AIOS dependency consumption, and a second consumer.
