...
healthScore: 85
statusLabel: warning
nextStep: Migrate the remaining session-ingestion and lower-traffic adapters, close the restored-copy retention review, then freeze the paired effectiveness corpus before later execution.
blockers: []
...
1. Migrate remaining managed-runtime and session-ingestion Python direct-connection adapters and remove UI request-time DDL only after deterministic migration/UI gates pass.
