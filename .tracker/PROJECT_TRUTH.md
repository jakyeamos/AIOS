...
## Recent Progress
- 2026-07-13: Routed `migrate-patterns.py` through shared storage (`8529dec`): honored `AIOS_DB`, passed Ruff/BasedPyright, and passed disposable schema/data migration proof.
- 2026-07-13: Routed the AIOS daily pipeline pending-rule query through shared read-only storage (`37dd07c`): honored `AIOS_DB`, passed Ruff/BasedPyright, and passed disposable `query_only=1` smoke.
- 2026-07-13: Routed the CLI doctor SQLite health probe through shared read-only storage (`dcc496a`): 2 doctor/health tests, Ruff, and BasedPyright passed.
- 2026-07-13: Routed iMessage and Apple Notes AIOS writes through shared storage (`6d5abe5`): preserved source readers/direct-script paths, passed Ruff and BasedPyright, and passed disposable contact/note write proof.
- 2026-07-13: Routed AI-history import and project inventory sync through shared storage (`ae19c1b`): `AIOS_DB`, direct-script import paths, Ruff, BasedPyright, and disposable ChatGPT/Git inventory proof passed.
- 2026-07-13: Routed metric recording, session evaluation, and lab reporting through shared storage (`88dfb43`): `AIOS_DB`, read-only report connections, Ruff, BasedPyright, and disposable write/read reporting integration passed.
- 2026-07-13: Routed stale-session repair, prompt sync, and workflow experiment utilities through shared storage (`cffbfe2`): direct-script imports/`AIOS_DB` defaults, 8 hook tests, Ruff, BasedPyright, and a disposable four-utility integration passed.
- 2026-07-13: Routed Codex, Claude, Cursor, and Antigravity canonical upserts through shared storage (`052eb51`): `AIOS_DB` defaults, 39 provider/intelligence tests, Ruff, BasedPyright, and FK-enforced disposable upsert proof passed.
- 2026-07-13: Routed `bin/sessions.py` through shared storage (`388891e`): honored `AIOS_DB`, preserved in-memory/missing read-only semantics, and passed 11 provider tests, Ruff, BasedPyright, and disposable pragma/schema checks.
- 2026-07-13: Routed `cron-ingest-codex.py` through shared storage (`941c85f`): `AIOS_DB` override, processed-file ledger smoke on a disposable store, 3 helper tests, Ruff, and BasedPyright passed.
- 2026-07-13: Routed all six managed-runtime main-store connection sites through shared storage (`f2a0d83`): handshake, TMCP shortcut, strategy routing, and closeout repair passed with `PYTHONPATH=.` focused tests plus Ruff and BasedPyright.
- 2026-07-13: Routed `hook-session-stop.py` through shared storage (`eb56b84`): repaired direct-script imports and passed Ruff, BasedPyright, and disposable peer-trace start/stop integration without touching live rows.
- 2026-07-13: Routed high-traffic prompt-submit, precompact, and post-tool-use hooks through shared storage (`194d27f`): canonical `AIOS_DB`/SQLite pragmas, 15 focused hook tests, Ruff, and BasedPyright passed without touching live rows.
- 2026-07-13: Routed utility adapters through shared storage (`659aa75`): issues, handoffs, read-only query, and statusline now honor `AIOS_DB` and canonical pragmas; 10 focused tests plus live read-only query smoke passed.
