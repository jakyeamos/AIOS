...
healthScore: 85
statusLabel: warning
nextStep: Migrate the remaining high-traffic Python direct-connection adapters, close the restored-copy retention review, then freeze the paired effectiveness corpus before later execution.
blockers: []
...
1. Migrate remaining high-traffic Python direct-connection adapters and remove UI request-time DDL only after deterministic migration/UI gates pass.
