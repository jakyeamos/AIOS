...
export async function compileContext({
  ...
  const contextRoutingManifest = buildContextRoutingManifest({
    task,
    taskId,
    runId,
    phase,
  ...
  const payload = {
    ...
    context_routing_manifest: contextRoutingManifest,
  };
      ...
      `${JSON.stringify(
        {
          ...
          context_routing_manifest: payload.context_routing_manifest,
...
function buildContextRoutingManifest({
  task,
  taskId,
  runId,
  phase,
  ...
  const normalizedAvailability =
    secondBrainAvailable === null ? inferSecondBrainAvailability() : Boolean(secondBrainAvailable);
  const loadedSources = selectedFiles.map((file) => ({
    id: file.frontmatter.id,
    path: file.relativePath,
    category: contextSourceCategory(file),
    reason: file.reason,
    token_estimate: file.token_cost_estimate,
  }));
  const skippedSources = skippedFiles.map((file) => ({
    id: file.id,
    path: file.path,
    category: contextSourceCategory(file),
    reason: file.reason,
  }));
  const touchesSecondBrain = classification.signals.includes("task_touches_knowledge_system")
    || classification.signals.includes("task_touches_obsidian")
    || normalize(task).includes("second brain");
  const secondBrainUsed =
    normalizedAvailability
    && loadedSources.some((source) => source.category === "retrieved_second_brain_context");
  const fallbackUsed = touchesSecondBrain && !normalizedAvailability;
  const caveats = [];
  if (fallbackUsed) {
    caveats.push("second-brain-unavailable: compiled peer-portable local context only");
  }
  return {
    task_id: taskId,
    run_id: runId,
    phase,
    context_sources_loaded: loadedSources,
    context_sources_skipped: skippedSources,
    retrieval_reasons: Object.fromEntries(
      loadedSources.map((source) => [source.id, source.reason]),
    ),
    estimated_context_tokens: selectedFiles.reduce(
      (total, file) => total + Number(file.token_cost_estimate || 0),
      0,
    ),
    second_brain_available: normalizedAvailability,
    second_brain_used: secondBrainUsed,
...
function inferSecondBrainAvailability() {
  if (process.env.AIOS_SECOND_BRAIN_AVAILABLE === "1") return true;
  if (process.env.AIOS_SECOND_BRAIN_AVAILABLE === "0") return false;
  return false;
}

function contextSourceCategory(file) {
  const id = String(file.id ?? file.frontmatter?.id ?? "");
  const tier = String(file.frontmatter?.tier ?? "");
  const sourcePath = String(file.path ?? file.relativePath ?? "");
  const searchable = normalize(`${id} ${sourcePath} ${file.frontmatter?.title ?? ""}`);
  if (id === AGENT_RULES_CONTEXT_ID) return "model_instructions";
  if (id === "handoffs.latest" || searchable.includes("handoff")) return "prior_run_lessons";
  if (searchable.includes("obsidian") || searchable.includes("knowledge")) {
    return "retrieved_second_brain_context";
  }
  if (tier === "global" || tier === "domain") return "stable_rules";
  if (tier === "project") return "project_rules";
  if (tier === "feature" || tier === "packet" || tier === "task") return "task_spec";
  return "repo_files";
}
...
function renderReceipt({
  ...
  return [
    ...
    "",
    `- Phase: ${contextRoutingManifest.phase}`,
    `- Loaded sources: ${contextRoutingManifest.context_sources_loaded.length}`,
    `- Skipped sources: ${contextRoutingManifest.context_sources_skipped.length}`,
    `- Estimated context tokens: ${contextRoutingManifest.estimated_context_tokens}`,
    `- Second brain available: ${contextRoutingManifest.second_brain_available}`,
    `- Second brain used: ${contextRoutingManifest.second_brain_used}`,
    `- Fallback used: ${contextRoutingManifest.fallback_used}`,
    "",
    "## Conflicts",
    "",
