...
def _build_patch_spec(artifact: dict) -> dict:
    ...
    scoped_specs = {
        ...
        "orchestration_flag": {
            "patch_type": "orchestration_flag",
            "payload": {"flags": {"MAX_TURNS": 40}},
        },
    }
    return scoped_specs.get(
        mutation_scope, {"patch_type": "prompt_overlay", "payload": {"text": rule_text}}
    )
...
def _get_eligible_patterns(conn: sqlite3.Connection, pattern_id: str | None) -> list[dict]:
    ...
    if pattern_id:
        rows = conn.execute("SELECT * FROM patterns WHERE id=?", (pattern_id,)).fetchall()
    else:
        rows = conn.execute(
...
def _update_pattern_post_run(
    conn: sqlite3.Connection, pattern_id: str, outcome: str, run_id: str
) -> None:
...
def run_experiment(
    ...
    core_task_names = (
        {d.name for d in TASKS_CORE.iterdir() if d.is_dir()} if TASKS_CORE.exists() else set()
    )
    hold_task_names = (
        {d.name for d in TASKS_HOLD.iterdir() if d.is_dir()} if TASKS_HOLD.exists() else set()
    )
    ...
    if bundle_id:
        micro_task_names = (
            {d.name for d in (TASKS_MICRO / bundle_id).iterdir() if d.is_dir()}
            if (TASKS_MICRO / bundle_id).exists()
            else set()
        )
    ...
    print(
        f"  candidate core={candidate_core:.3f}  holdout={candidate_hold:.3f}"
    ...
    micro_delta = (
        (candidate_micro - 0.0) if candidate_micro is not None else None
    )  # baseline micro=0 (no prior run)
...
def main():
    ...
    group.add_argument(
        "--all", action="store_true", help="Run experiments for all eligible patterns"
    )
