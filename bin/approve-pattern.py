...
def list_approvable(conn: sqlite3.Connection) -> None:
    ...
    for r in rows:
        ...
        print(
            f"{pid}  {(domain or '?'):12} {(class_ or '?'):12} {conf:.2f}  {cnt or 0:3}  {title[:50]}"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--all-knowledge",
        action="store_true",
        help="Interactive approval of all knowledge patterns",
    )
    parser.add_argument(
        "--force", action="store_true", help="Skip gate checks (document reason manually)"
    )
