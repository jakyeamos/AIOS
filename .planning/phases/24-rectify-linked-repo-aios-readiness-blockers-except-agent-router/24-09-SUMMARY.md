...
This was superseded after closeout when the user clarified that GitHub Actions credits are constrained and selected the local CI replacement policy. `config/quality-pipeline.json` now records `non_remote_ci_exception` objects for all 21 in-scope repos. Workflow-file presence remains insufficient by itself; every in-scope repo remains blocked until the local replacement `ci` proof passes.
