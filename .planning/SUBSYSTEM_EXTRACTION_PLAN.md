...
| Agent harness and eval run recording | `/Users/jakyeamos/agent-eval-contract`, `services/harness*.py`, `services/eval_run_service.py`, `docs/evals/`, `config/agent-eval/` | `core_aios` for runtime/storage; external `contract_package` dependency for schemas/templates | 2026-06-27 split moved portable `agent_eval_contract` code to `/Users/jakyeamos/agent-eval-contract`; remote `git@github.com:jakyeamos/agent-eval-contract.git` is tagged `v0.1.0` at `7489155`. AIOS consumes it through `agent-eval-contract @ file:///Users/jakyeamos/agent-eval-contract`; the package owns context profiles, status/priority validation, score fields, TypedDict schemas, harness fixture validation, eval template validation, sample record validation, clean-room contract checks, non-AIOS fixture production, release metadata, and external result normalization. | Keep runtime, storage, and CLI integration inside AIOS. Do not reintroduce portable contract code into AIOS. | Decide when to switch from local path to tagged dependency consumption and establish a second real consumer. |
...
| Agent eval framework | `/Users/jakyeamos/agent-eval-contract`, `docs/evals/`, `config/agent-eval/`, `services/harness_eval.py`, `services/external_benchmark_adapter.py` | `repo-extracted-remote` | `share-contract` with success criteria and quality findings | Portable schemas/templates/fixtures now live in `/Users/jakyeamos/agent-eval-contract`; remote `jakyeamos/agent-eval-contract` exists with pushed `v0.1.0`. Keep AIOS runtime in AIOS. | Eval storage, peer traces, shadow worktrees, second-brain lift, and operator projections are AIOS-local. The extracted package still lacks tagged AIOS dependency consumption and a second consumer beyond AIOS/tests. | Prove tagged dependency consumption and reuse vocabulary with `quality-evidence-contract`. |
...
| --- | --- | --- | --- |
| 2026-07-13 | Select a parallel v2 implementation with progressive migration and one state/mutation authority; retain runtime-coupled subsystems in AIOS, keep CTS/business memory as sidecars, and consume extracted contracts/tools through adapters. | `docs/modernization/ADR-005-subsystem-ownership-and-parallel-v2-strategy.md`, `CTS_BOUNDARY_AUDIT.md`, `EVAL_BENCHMARK_BOUNDARY_AUDIT.md`, ADR-001/002/003/004, and the current ownership map. | Ticket 006 must turn this posture into dependency-ordered vertical slices with explicit migration, rollback, and deletion criteria. |
| 2026-07-01 | Treat daily-use release readiness as core AIOS product behavior and pause linked-repo portfolio certification as the release centerpiece. | `doctor --json`, direct CLI help, integrated `start-work -> daily-flow -> next-action` tests, pnpm-only UI CI, README daily-use path, and `docs/case-study.md` now define the immediate release proof. | Resume Phase 29 portfolio proof only after AIOS self-readiness gates stay green and the daily-use loop is the documented first path. |
...
| 2026-06-27 | Added non-AIOS fixture production and release metadata for `agent_eval_contract`. | `agent_eval_contract.fixture_runner` writes sample JSON, generated template Markdown, and a manifest from any working directory, while `agent_eval_contract.release` validates the portable-vs-AIOS-owned release boundary from checked-in metadata. | Run a physical split rehearsal, prove tagged AIOS consumption, and find or create a second consumer before extracting the package to its own repo. |
| 2026-06-27 | Physically split `agent_eval_contract` into `/Users/jakyeamos/agent-eval-contract`. | The external repo has initial commit `7489155`, remote `git@github.com:jakyeamos/agent-eval-contract.git`, pushed `main`, pushed tag `v0.1.0`, standalone package metadata, console fixture runner, sample/template/release metadata tests, Pre-CR config, and passing standalone checks. AIOS removed the in-tree package and imports `agent-eval-contract` through a local file dependency. | Prove tagged AIOS dependency consumption and find or create a second consumer before considering the split fully release-complete. |
