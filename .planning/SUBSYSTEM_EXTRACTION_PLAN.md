...
| Agent harness and eval run recording | `services/harness*.py`, `services/eval_run_service.py`, `docs/evals/`, `config/agent-eval/` | `core_aios` for runtime/storage; `contract_package` candidate for schemas/templates | 2026-06-26 audit rejects a standalone runtime split. AIOS needs eval records as operating-layer evidence tied to SQLite, runs, context profiles, success criteria, closeout, and operator surfaces. | Keep runtime, storage, and CLI integration inside AIOS; promote only schemas, context-profile vocabulary, failure taxonomy, fixture formats, and templates toward a future contract package. | Add validators and non-AIOS fixture consumption for eval records, harness fixtures, and template artifacts before creating an `agent-eval-contract` package. |
...
| Agent eval framework | `docs/evals/`, `config/agent-eval/`, `services/harness_eval.py`, `services/external_benchmark_adapter.py` | `bounded` | `share-contract` with success criteria and quality findings | Candidate for a narrow `agent-eval-contract` package later; keep runtime in AIOS. | Eval storage, peer traces, shadow worktrees, second-brain lift, and operator projections are AIOS-local. Generic schemas/templates do not yet have package validators or a non-AIOS producing runner. | Reusable schema/template validators, sample eval records, clean-room fixture run, and explicit vocabulary sharing with `quality-evidence-contract`. |
