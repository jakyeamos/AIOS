...
| CTS repository intelligence | `services/cts/**`, `data/cts/`, CTS CLI/MCP entrypoints | `incubator_candidate` | Questionable permanent scope. It supports AIOS context and impact analysis but could become a repo-intelligence service. | Keep inside until graph-store API and query contract are stable. | Produce isolated index/search fixtures and decide `standalone_tool` vs `core_aios` sidecar. |
...
| Agent harness and eval run recording | `services/harness*.py`, `services/eval_run_service.py`, `docs/evals/`, `config/agent-eval/` | `incubator_candidate` | Partly in scope. AIOS needs evals, but reusable benchmark/eval harness pieces may not be core. | AIOS-owned run recording stays; reusable schemas/templates should be contract candidates. | Split eval record schema from local DB writes in fixtures. |
| External benchmark and corpus evaluation | `services/external_benchmark_adapter.py`, `services/tmcp_benchmark.py`, `tmcp-benchmark/`, `config/peer-eval/` | `incubator_candidate` | Questionable permanent scope. It is useful for AIOS proving, but likely reusable tooling later. | Keep until benchmark evidence model stabilizes. | Decide standalone benchmark tool vs AIOS-only proving harness after repeated benchmark runs. |
| Personalized humanizer | `services/personalized_humanizer.py`, `config/personalized-humanizer/`, `skills/personalized-humanizer/` | `incubator_candidate` leaning `standalone_tool` | Not obviously core unless positioned as AIOS memory/persona behavior. | Keep temporarily; do not deepen coupling to unrelated AIOS runtime. | Decide whether it is an AIOS memory projection or separate writing product. |
