#!/usr/bin/env python3
"""Freeze a receipt-backed control/treatment pair before launching Codex."""
...
ACCEPTANCE = [
    "validated Python service/CLI and typed server adapter own the mutation",
    "success, missing candidate, invalid payload, and state-transition tests pass",
    "UI lint/typecheck, architecture, build, and relevant browser checks pass or are explicitly recorded as environment-blocked",
    "source scan proves eval-data/router no longer owns the update",
...
def main() -> int:
    ...
    if artifact_root.exists():
        raise SystemExit(f"Refusing to overwrite existing artifact root: {artifact_root}")
    (artifact_root / "prompts").mkdir(parents=True)
    (artifact_root / "receipts").mkdir()

    control_source = ROOT / "benchmark/m6-promotion-prompts/task-b-rerun-control.md"
    treatment_source = ROOT / "benchmark/m6-promotion-prompts/task-b-rerun-treatment.md"
    control_snapshot = artifact_root / "prompts/task-b-control.md"
    treatment_snapshot = artifact_root / "prompts/task-b-treatment.md"
    ...
    context_entries = [{"path": path, "sha256": digest(protected_bytes(path)), "byte_count": len(protected_bytes(path))} for path in CONTEXT_FILES]
    context_manifest_hash = json_digest([[entry["path"], entry["sha256"]] for entry in context_entries])
    task_hash = json_digest({"slug": "task-b-shadow-approval-owner", "acceptance": ACCEPTANCE})
    created_at = datetime.now(UTC).isoformat()
    tools = ["shell", "rg", "sed", "git", "python3", "pnpm", "pytest", "playwright"]
    common: dict[str, Any] = {
        "schema": "aios-m6-b-pre-run-receipt-v1",
        "created_at_utc": created_at,
        "protected_start_sha": SHA,
        "task_slug": "task-b-shadow-approval-owner",
        "task_hash": task_hash,
        "model": "gpt-5.6-luna",
    ...
    specs = (
        ...
        ("treatment", args.treatment, treatment_snapshot, "peer_portable_context_packet", "m6-owner-migrations-v1"),
    )
    for condition, worktree, prompt_snapshot, context_profile, packet_id in specs:
        ...
        label = "b2-control" if condition == "control" else "b2-treatment"
        receipt_id = f"m6-b-rerun-{condition}"
        ...
        receipt = {
            **common,
            ...
            "launch_command_argv": ["codex", "exec", "--json", "--model", "gpt-5.6-luna", "-c", "model_reasoning_effort=high", "-s", "workspace-write", "-C", str(worktree), "-o", str(artifact_root / f"{label}-final.md"), "-",],
        ...
        event_path.write_text(json.dumps({"type": "benchmark.pre_run_receipt", "receipt_id": receipt_id, "receipt_path": str(receipt_path), "receipt_sha256": receipt_sha, "protected_start_sha": SHA, "task_hash": task_hash, "prompt_sha256": receipt["prompt_sha256"], "packet_sha256": receipt["packet_sha256"], "context_manifest_sha256": context_manifest_hash, "condition": receipt["condition"]}) + "\n", encoding="utf-8")
        print(json.dumps({"condition": condition, "receipt": str(receipt_path), "receipt_sha256": receipt_sha, "events": str(event_path), "prompt_sha256": receipt["prompt_sha256"], "packet_sha256": receipt["packet_sha256"], "context_manifest_sha256": context_manifest_hash}, sort_keys=True))
    return 0
